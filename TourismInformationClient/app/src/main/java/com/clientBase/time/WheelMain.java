	...
	public String getTime() {
		...
		if (!hasSelectTime){
			...
			if(wv_month.getCurrentItem()<9){
				monthMsg = "0"+(wv_month.getCurrentItem() + 1);
			}else{
				monthMsg = ""+(wv_month.getCurrentItem() + 1);
			}
			
			if(wv_day.getCurrentItem()<9){
				dayMsg = "0"+(wv_day.getCurrentItem() + 1);
			}else{
				dayMsg = ""+(wv_day.getCurrentItem() + 1);
			}
			
			
			sb.append((wv_year.getCurrentItem() + START_YEAR)).append("-")
					.append(monthMsg).append("-")
					.append(dayMsg);
		}
