...
public class OfforShopAdapter extends BaseAdapter {
	private LayoutInflater inflater;
	private List<OfforModel> list_result;
	private int posIndex;
	private Context mContext;

	public OfforShopAdapter(Context context, List<OfforModel> list_result) {
		mContext = context;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.newsShowTitle.setText(list_result.get(position).getOfferUserName()+"出示金额是"+list_result.get(position).getOfferMoney()+"元");
		holder.newsShowMessage.setText("确认时间：" + list_result.get(position).getOfferTime());
		return convertView;
