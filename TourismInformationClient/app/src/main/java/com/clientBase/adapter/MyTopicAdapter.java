...
public class MyTopicAdapter extends BaseAdapter {
    ...
    public MyTopicAdapter(Context context, List<TopicModel> list_result) {
        mContext = context;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.item_mytopic, null);
            ...
            holder.gameState = (TextView) convertView.findViewById(R.id.gameState);
        ...
        if(list_result.get(position).getTopicFlag().equals("1")){
            holder.gameState.setText("待审核");
        }else if(list_result.get(position).getTopicFlag().equals("2")){
            holder.gameState.setText("已批准");
        }else if(list_result.get(position).getTopicFlag().equals("3")){
            holder.gameState.setText("已拒绝");
        }
