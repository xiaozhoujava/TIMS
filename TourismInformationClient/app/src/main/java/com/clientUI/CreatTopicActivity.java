...
public class CreatTopicActivity extends BaseActivity implements ImageItemClickListner {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_creat_imginfor);
		initWidget();
		initData();
	}
	...
	public void initWidget() {
		...
		dialogListMsg.setTitle().setText("请选择话题");
...
//		listPay(false);
	...
	public void initData() {
		...
		topicConclusion.setText("曾经输掉的东西，只要你想，就可以一点一点再赢回来。");
//		listHuaTiType(false);
	...
	private void initSelectedGridView() {
		...
		grid_instructor.setGridAdapter(selectedImageAdapter, CreatTopicActivity.this);
	...
	public void imageItemClick(int position, SelectImageItem imageItem) {
		if (imageItem != null) {
			...
			if (sid == 100) {
				...
				if (CreatTopicActivity.this.getIntent().getIntExtra("photo_message", 0) == 1) {
					goCameraActivity();
				} else {
					// 添加图片
					Intent intentImages = new Intent(CreatTopicActivity.this, SelectImagesActivity.class);
					intentImages.putExtra("image_count", imageItems.size());
					intentImages.putExtra("max_count", "1");
					startActivityForResult(intentImages, 1);
				}
			} else {
				Intent intentPicture = new Intent(CreatTopicActivity.this, ShowCreatePicturesActivity.class);
	...
	private void createTopicPost(boolean isShow) {
		...
		params.put("action_flag", "addTopic");

		params.put("topicTypeInfor", mtvStart.getText().toString()+"");
		params.put("topicInfor",  topicConclusion.getText().toString());

		if (mListImage.size() != 0) {
			params.put("topicImg", imagePath.substring(0, imagePath.length() - 1));
		}
...
//				TopicObservable.getInstance().notifyStepChange("ok");
				ToastUtil.ShowToast(CreatTopicActivity.this, entry.getRepMsg());
				new Handler().postDelayed(new Runnable() {
					//
					@Override
					public void run() {
	...
	protected void callBackAllFailure(String strMsg, int actionId) {
		...
		mdialog.dismiss();
		ToastUtil.show(CreatTopicActivity.this, strMsg);
		...
		protected void onPostExecute(String result) {
			...
			if (UploadUtils.SUCCESS.equalsIgnoreCase(result)) {
				if (imageFlagNumber < mListImage.size()) {
					UploadFileTask uploadFileTask = new UploadFileTask(CreatTopicActivity.this);
