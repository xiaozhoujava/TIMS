...
public class CreatActivity extends BaseActivity implements ImageItemClickListner{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_creat_green);
		initWidget();
		initData();
	}
	...
	public void initWidget() {
		...
		mTvTitle.setText("添加发布景点");
		mIvBack.setVisibility(View.VISIBLE);
		mIvBack.setOnClickListener(this);
		mSubmit.setOnClickListener(this);

		shopName.setText("大雁塔");
		shopAddress.setText("陕西省西安市雁塔区大慈恩寺内");
		shopMoney.setText("50");
		shopMessage.setText("大雁塔作为现存最早、规模最大的唐代四方楼阁式砖塔，是佛塔这种古印度佛寺的建筑形式随佛教传入中原地区，并融入华夏文化的典型物证，是凝聚了中国古代劳动人民智慧结晶的标志性建筑");
	...
	private void createTopicPost(boolean isShow) {
		...
		params.put("action_flag", "addShopPhone");
	...
	protected void callBackSuccess(ResponseEntry entry, int actionId) {
		...
		switch (actionId) {
			case Consts.actionId.resultCode:
				...
				ToastUtil.ShowToast(CreatActivity.this, entry.getRepMsg());
				new Handler().postDelayed(new Runnable() {
					//
					@Override
					public void run() {
	...
	public class UploadFileTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			return UploadUtils.uploadFile(file, Consts.URL + Consts.APP.MessageAction + "?action_flag=addShopPhone");
		}
